...
const useBorrowPositions = (connection, wallet, honeyId, honeyMarketId) => {
    const [status, setStatus] = (0, react_1.useState)({ loading: false });
    const { assetStore } = (0, honey_1.useHoney)();
    const { honeyUser } = (0, useMarket_1.useMarket)(connection, wallet, honeyId, honeyMarketId);
    const fetchData = async () => {
        ...
        const promises = collateralNftMint.map(async (key, index) => {
            if (!key.equals(web3_js_1.PublicKey.default)) {
                const [nftMetadata, metadataBump] = await web3_js_1.PublicKey.findProgramAddress([
