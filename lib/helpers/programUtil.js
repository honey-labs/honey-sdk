...
const honeyTypes_1 = require("./honeyTypes");
...
const sendTransaction = async (provider, instructions, signers, skipConfirmation) => {
    ...
    try {
        ...
        console.log('Signing Transactions Failed', err, [honeyTypes_1.TxnResponse.Failed, null]);
        // wallet refused to sign
        return [honeyTypes_1.TxnResponse.Cancelled, []];
    }
    // }
    // Sending phase
...
const parseIdlMetadata = (idlMetadata) => {
    return {
        ...
        market: {
            market: new web3_js_1.PublicKey(idlMetadata.market.market),
            marketAuthority: new web3_js_1.PublicKey(idlMetadata.market.marketAuthority)
        },
