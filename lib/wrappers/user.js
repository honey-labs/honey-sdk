...
class HoneyUser {
    ...
    async withdrawNFT(tokenAccount, tokenMint, updateAuthority) {
        ...
        try {
            ...
            return [honeyTypes_1.TxnResponse.Success, [txid]];
        }
        catch (err) {
    ...
    async makeNFTWithdrawTx(tokenAccount, tokenMint, updateAuthority) {
        ...
        this.reserves.forEach((reserve) => {
            if (!reserve.address.equals(web3_js_1.PublicKey.default))
                tx.add(reserve.makeRefreshIx());
        });
        tx.add(await this.client.program.instruction.withdrawNft(metadataBump, {
            accounts: {
                market: this.market.address,
                marketAuthority: this.market.marketAuthority,
    ...
    async makeNFTDepositTx(tokenAccount, tokenMint, updateAuthority) {
        ...
        if (!collateralData) {
            ...
            try {
                ...
                return [honeyTypes_1.TxnResponse.Failed, txids];
            }
        }
        const tx = new web3_js_1.Transaction();
        ...
        try {
            const txid = await this.client.program.provider.send(tx);
            txids.push(txid);
            return [honeyTypes_1.TxnResponse.Success, txids];
        }
        catch (err) {
    ...
    async makeBorrowTx(reserve, receiver, amount) {
        ...
        const [feeReceiverAccount, feeReceiverAccountBump] = await web3_js_1.PublicKey.findProgramAddress([Buffer.from('honey-protocol-fee'), reserve.data.tokenMint.toBuffer()], this.client.program.programId);
        const [loanAccountPK, loanAccountBump] = await web3_js_1.PublicKey.findProgramAddress([Buffer.from('loan'), reserve.address.toBuffer(), this.obligation.address.toBuffer(), this.address.toBuffer()], this.client.program.programId);
        const borrowSeeds = {
        ...
        const ixs = [
            {
                ix: [createTokenAccountIx, createWsolTokenAccountIx, initWsoltokenAccountIx, initLoanAccountIx].filter((ix) => ix),
                signers: [wsolKeypair].filter((ix) => ix),
            },
            {
                ix: [...refreshReserveIxs, borrowIx, closeTokenAccountIx].filter((ix) => ix),
            },
        ];
        return ixs;
    }
