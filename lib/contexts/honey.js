...
const honeyProtocolHelpers_1 = require("../helpers/honeyProtocolHelpers");
...
const HoneyProvider = ({ children, wallet, connection, honeyProgramId, honeyMarketId }) => {
    const { program, coder, isConfigured } = (0, anchor_1.useAnchor)();
    const { honeyMarket, honeyReserves } = (0, hooks_1.useMarket)(connection, wallet, honeyProgramId, honeyMarketId);
    ...
    (0, react_1.useEffect)(() => {
        const fetchReserves = async () => {
            const reserveStructures = await (0, honeyProtocolHelpers_1.getReserveStructures)(honeyReserves);
            setMarket(current => ({
                ...current,
    ...
    (0, react_1.useEffect)(() => {
        ...
        const subscribeToMarket = async () => {
            let promise;
            const promises = [];
            promise = (0, programUtil_1.getAccountInfoAndSubscribe)(program.provider.connection, honeyMarket.address, account => {
                if (account != null) {
            ...
            for (const key in market.reserves) {
                ...
                // Reserve
                promise = (0, programUtil_1.getAccountInfoAndSubscribe)(program.provider.connection, reserveMeta.accountPubkey, account => {
                    if (account != null) {
                ...
                // Deposit Note Mint
                promise = (0, programUtil_1.getMintInfoAndSubscribe)(program.provider.connection, reserveMeta.depositNoteMintPubkey, amount => {
                    if (amount != null) {
                        const reserve = market.reserves[reserveMeta.abbrev];
                ...
                promises.push(promise);
                // Loan Note Mint
                promise = (0, programUtil_1.getMintInfoAndSubscribe)(program.provider.connection, reserveMeta.loanNoteMintPubkey, amount => {
                    if (amount != null) {
                        const reserve = market.reserves[reserveMeta.abbrev];
                ...
                promises.push(promise);
                // Reserve Vault
                promise = (0, programUtil_1.getTokenAccountAndSubscribe)(program.provider.connection, reserveMeta.vaultPubkey, reserveMeta.decimals, amount => {
                    if (amount != null) {
                        const reserve = market.reserves[reserveMeta.abbrev];
                ...
                promises.push(promise);
                // Reserve Token Mint
                promise = (0, programUtil_1.getMintInfoAndSubscribe)(program.provider.connection, reserveMeta.tokenMintPubkey, amount => {
                    if (amount != null) {
                        const reserve = market.reserves[reserveMeta.abbrev];
                ...
                promises.push(promise);
                // Pyth Price
                promise = (0, programUtil_1.getAccountInfoAndSubscribe)(program.provider.connection, reserveMeta.pythPricePubkey, account => {
                    if (account != null) {
                        const reserve = market.reserves[reserveMeta.abbrev];
    ...
    (0, react_1.useEffect)(() => {
        const fetchAssets = async () => {
            const fetchedAssetStore = await (0, honeyProtocolHelpers_1.getAssetPubkeys)(market, user, program, wallet);
    ...
    return (react_1.default.createElement(HoneyContext.Provider, { value: honeyContext }, children));
};
